...
export const unsuspendCompany = createAsyncThunk<Company, string, { rejectValue: string }>(
    'companies/unsuspend',
    async (companyId, { rejectWithValue }) => {
        try {
            const res = await companyService.unsuspendCompany(companyId)
            return res.data.data
        } catch (error: unknown) {
            return rejectWithValue(getErrorMessage(error))
        }
...
const companiesSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(unsuspendCompany.pending, (state) => {
                state.loading = true;

            })
            .addCase(unsuspendCompany.fulfilled, (state, action) => {
                state.loading = false;
                const index = state.items.findIndex(c => c.id === action.payload.id)
                if (index !== -1) {
                    state.items[index] = action.payload
            ...
            .addCase(unsuspendCompany.rejected, (state, action) => {
                state.loading = false;
                state.error = action.payload ?? 'Failed to unsuspend company'
            })
    }
});
